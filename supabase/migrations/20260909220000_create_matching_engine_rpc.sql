-- =============================================================================
-- Migration: 20260909220000_create_matching_engine_rpc.sql
-- Description: Recommendation & Matching Engine using Weighted Jaccard Similarity
--              with Progressive Location Gateway and Anti-Spam Safeguards.
-- =============================================================================
...
BEGIN
    ...
    IF p_apply_location_filter IS TRUE AND v_user_city IS NOT NULL AND TRIM(v_user_city) <> '' THEN
        IF v_user_barangay IS NOT NULL AND TRIM(v_user_barangay) <> '' THEN
            ...
            WHERE (v_user_id IS NULL OR ua."ACCOUNT_ID" <> v_user_id)
              ...
              AND a."Status" = 'Active'
              AND LOWER(TRIM(ua."City")) = LOWER(TRIM(v_user_city))
              AND LOWER(TRIM(ua."Barangay")) = LOWER(TRIM(v_user_barangay));
    ...
    WITH candidate_base AS (
        SELECT
            ...
            CASE
                WHEN v_selected_loc_level = 'Barangay' THEN 'Barangay'
                WHEN v_selected_loc_level = 'City' THEN 'City'
                ELSE 'Global'
            END AS c_loc_level,
        ...
        WHERE (v_user_id IS NULL OR ua."ACCOUNT_ID" <> v_user_id)
          ...
          AND a."Status" = 'Active'
          AND (
              v_selected_loc_level = 'Global'
              OR (v_selected_loc_level = 'Barangay' AND LOWER(TRIM(ua."City")) = LOWER(TRIM(v_user_city)) AND LOWER(TRIM(ua."Barangay")) = LOWER(TRIM(v_user_barangay)))
              OR (v_selected_loc_level = 'City' AND LOWER(TRIM(ua."City")) = LOWER(TRIM(v_user_city)))
          )
    ),
    similarity_calc AS (
        SELECT
    ...
    scored_candidates AS (
        SELECT
            ...
            COALESCE((SELECT ARRAY_AGG(tg."Name") FROM public."TAG_GENRE" tg WHERE tg."Genre_ID" = ANY(sc.c_genre_ids)), ARRAY[]::TEXT[]) AS tag_genres,
            COALESCE((SELECT ARRAY_AGG(ti."Name") FROM public."TAG_INSTRUMENT" ti WHERE ti."Instrument_ID" = ANY(sc.c_inst_ids)), ARRAY[]::TEXT[]) AS tag_insts,
            COALESCE((SELECT ARRAY_AGG(tg."Name") FROM public."TAG_GENRE" tg WHERE tg."Genre_ID" = ANY(sc.g_inter)), ARRAY[]::TEXT[]) AS shared_g,
            COALESCE((SELECT ARRAY_AGG(ti."Name") FROM public."TAG_INSTRUMENT" ti WHERE ti."Instrument_ID" = ANY(sc.i_inter)), ARRAY[]::TEXT[]) AS shared_i,
