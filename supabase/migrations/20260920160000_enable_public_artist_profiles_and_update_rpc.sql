-- =============================================================================
-- Migration: 20260920160000_enable_public_artist_profiles_and_update_rpc.sql
-- Description: 
--   1. Enables public read RLS policies for USER_ACCOUNT, ARTIST, SOLO_ARTIST,
--      BAND, and tag junction tables to allow unauthenticated/public visitors
--      and social media scrapers (Open Graph) to view public artist profiles.
--   2. Updates tono_match_recommendations RPC to include the artist's username
--      so artist cards in feeds can link directly to /artist/:username.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. PUBLIC READ RLS POLICIES
-- -----------------------------------------------------------------------------

-- Public can read non-banned user accounts (for username, city, avatar, cover)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'USER_ACCOUNT' 
        AND policyname = 'Public can read public user accounts'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        AND policyname = 'Public can view active artists'
    ) THEN
...
-- Public can view solo artist records
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'SOLO_ARTIST' 
        AND policyname = 'Public can view solo artists'
    ) THEN
        CREATE POLICY "Public can view solo artists" 
        ON public."SOLO_ARTIST"
        FOR SELECT 
        USING (true);
    END IF;
END $$;

-- Public can view band records
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'BAND' 
        AND policyname = 'Public can view bands'
    ) THEN
...
-- Public can view solo instruments
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
...
-- Public can view account preferred genres (fallback tags)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
...
-- Public can view account preferred instruments (fallback tags)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
...
-- -----------------------------------------------------------------------------
-- 2. UPDATE tono_match_recommendations RPC TO RETURN ARTIST USERNAME
-- -----------------------------------------------------------------------------
