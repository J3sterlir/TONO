...
onMounted(async () => {
  ...
  if (!error) {
    genreOptions.value = (data ?? []).map((row: { Name: string }) => row.Name).filter(Boolean)
  }
})
...
<template>
    <div class="text-white w-full max-w-md flex flex-col gap-2">
        <h1 class="text-center font-bold text-[22px] mt-5">Select Genres</h1>
        <p class="font-light">Select the genres that best define your sound. *</p>
        
        <p class="text-[#C7C5CE]/70 font-light text-[15px] mt-5">SELECT GENRE TAGS</p>
        <div v-if="genreOptions.length" class="text-white text-center grid grid-cols-3 gap-x-3 gap-y-3">
            <button
        ...
        <div v-else class="mt-2 text-sm text-[#B0B4D7]">Loading genres...</div>
